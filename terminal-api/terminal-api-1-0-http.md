...
| Terminal | 端末系：残高照会 | /v1/terminal/actions/inquireBalance | POST | |
| Terminal | 端末系：チャージ | /v1/terminal/actions/charge | POST |  |
| Terminal | ソフトウェア更新 | /v1/terminal/actions/softUpdate | POST | |
| Terminal | 保守メニュー表示 | /v1/terminal/actions/showMaintenance | POST | |
...
- 定義されていない URL にアクセスした際は、以下のエラーを返却する。

  `status code : 404 , error code : NOT_FOUND`

### アクティベーション

- すべてのAPIは利用する際、事前に端末がアクティベーションされている必要がある。  
  もし、未アクティベーションの場合、以下のエラーを返却する。

  `status code : 400 , error code : NOT_ACTIVATED`

  なお、未アクティベーションの場合 TerminalAPI を立ち上げると Maintenanceアプリを立ち上げようとする挙動となる。
...
- 決済関連のAPIは "開局状態" というステータスを参照する。  
  開局状態が "開局" でなければ、APIの実行はできない。
...
- 開局後24時間経過すると、自動的に "期限切れ" に更新される。


### 自動再開局

- アプリケーション設定で "前回の開局から24時間以内に自動で再開局" を設定すると、
  タイマー処理により24時間後に再処理が行われる。

  ※ これは起動時と同様 paymentInitializerUseCase により実行される。

- 設定が無効であれば、再開局は実行されない。  
  その場合、運用としては API : 再開局" の呼び出しか、アプリケーションの再起動を実行しなければ決済処理の制限が入る。
   

### 業務状態

- 開局と似た状態で "業務状態" というものがある。

- こちらは 起動時に "業務終了" となり、起動時のブート処理を完了すると "業務開始" となる。
...
- `history_aggregate` というテーブルがあるが、これは 「起動」～「業務終了APIコール」 までの時刻が記録される。
  "開局状態" ではないし、自動記録もされないことに注意。
...
No CVM 設定では、PIN入力は行われない。（No CVM とは本人確認無しという意味）
